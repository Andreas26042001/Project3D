...
public:
	...
	Object(const char* path) {
		...
		while (std::getline(infile, line))
		{
			...
			if (indice == "f") {
				...
				if (isQuad) {
					// parse f4
					std::string p4 = f4.substr(0, f4.find("/"));
					f4.erase(0, f4.find("/") + 1);

					std::string t4 = f4.substr(0, f4.find("/"));
					f4.erase(0, f4.find("/") + 1);

					std::string n4 = f4.substr(0, f4.find("/"));

					Vertex v4;
					v4.Position = positions.at(std::stof(p4) - 1);
					v4.Normal = normals.at(std::stof(n4) - 1);
					v4.Texture = textures.at(std::stof(t4) - 1);
	...
	void makeObject(Shader shader, bool texture = true) {
		...
		*/
		...
		if (texture) {
			auto att_tex = glGetAttribLocation(shader.ID, "aTexCoords");
